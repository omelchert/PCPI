...
def modelSourceVolume():
        ...
        """
        # SOURCE VOLUME PARS --------------------------------------------------
        xMax, Nx = 0.5, 600     # bdry, meshpts: x-axis
        yMax, Ny = 0.5, 600     # bdry, meshpts: y-axis
        zMax, Nz = 0.12, 80     # bdry, meshpts: z-axis

        # ABSORBING LAYER PARS ------------------------------------------------
        z0, dz = 0.0, 0.1       # start, width of absorbing layer        
        mu = 200.0               # absorption coeffiecient

        # FLAT TOP BEAM PROFILE  PARS -----------------------------------------
        x0, y0 = xMax/2, yMax/2 # x,y pos of symmetry axis
        fta, ftr = 0.085, 0.085/0.015  # radius, radius/edge width ratio
        f0 = 0.3 # incident fluence (J/cm^2)
        ...
        return (x,y,z), (x0,y0,dz), roi*10**6
...
def main():
        ...
        tp = 3.*10**(-8) # gaussian pulse width (s) 
        p = np.gradient(p)
        pf = pp.convolveGauss(p,(t,tp))

        # LIST RESULTS --------------------------------------------------------
        tau = t + zD/c0 # retarded time
        print "# (t in s) (c0 tau in cm) (p(tau) in bar)"
        for i in range(tau.size):
               print t[i], c0*tau[i], pf[i]


main()
# EOF: pureAbs_useCase1_main.py 
