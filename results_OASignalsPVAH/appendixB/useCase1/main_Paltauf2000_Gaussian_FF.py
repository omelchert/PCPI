...
def modelSourceVolume():
        ...
        """
        # SOURCE VOLUME PARS --------------------------------------------------
        xMax, Nx = 3., 1200    # bdry, meshpts: x-axis
        yMax, Ny = 3., 1200    # bdry, meshpts: y-axis
        zMax, Nz = 0.3, 100     # bdry, meshpts: z-axis

        # ABSORBING LAYER PARS ------------------------------------------------
        z0, dz = 0.0, 0.30      # start, width of absorbing layer        

        # FLAT TOP BEAM PROFILE  PARS -----------------------------------------
        x0, y0 = xMax/2, yMax/2 # x,y pos of symmetry axis
        a0 = float(sys.argv[2])/np.sqrt(2) # radius, edge width
        f0 = 1.00 # incident fluence (J/cm^2)
...
def main():
        ...
        t, p = ps.pressure(((x,y,z),Wxyz),(x0,y0,z0+zD),c0)
        norm = 1./max(p)
