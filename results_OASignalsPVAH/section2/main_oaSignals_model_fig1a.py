...
def modelSourceVolume():
        ...
        """
        # SOURCE VOLUME PARS --------------------------------------------------
        xMax, Nx = 1.0, 400    # bdry, meshpts: x-axis
        yMax, Ny = 1.0, 400    # bdry, meshpts: y-axis
        zMax, Nz = 1.0, 150     # bdry, meshpts: z-axis
...
def detectorPositions((x0,z0),RD,N=100):
        (phiList,dPhi) = np.linspace(0.,2*np.pi,N, endpoint=False,retstep=True)
        pos = [(np.degrees(phi),x0+RD*np.cos(phi),z0-RD*np.sin(phi))   for phi in phiList]
        return pos
...
def sinogram():
        ...
        detPos = detectorPositions((x0,z0+0.2),R,100)
